...
- (void)run
{
    ...
    if ([[self class] instanceMethodForSelector:_cmd] == [[CCActivity class] instanceMethodForSelector:_cmd]) {
        ...
        //      --OR--
        //    * Make sure your CCActivity subclass implements/overrides this run method (and calls [super run] first thing)
        //    * -[CCAppController setupApplication] ultimately needs to be invoked once on startup (usually from your subclassed run)
        //    * (You may wish to generate a new test project from SpriteBuilder as a reference to trace the differences)
